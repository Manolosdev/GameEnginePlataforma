...
public abstract class EntidadeQueMove extends Entidade {
    ...
    // Velocidade - quantdade de pixels que a entidade se move a cada tick.
    Point2D.Double speed;
    // Aceleração - taxa pela qual a velocidade aumenta a cada tick.
    Point2D.Double accel;
    // Velocidade máxima permitidade.
    Point2D.Double maxSpeed;
    // Fricção - constante que diminui a velocidade horizontal(faz parar).
    double friction;
    ...
    public void update(int currentTick) {
        // Para fazer o objeto andar basta informar a aceleração (accel) antes de char este update.
        ...
        if (speed.y < 0) {
            // Se a velocidade é menor que zero,o objeto está subindo
            // Testa colisão na direção acima.
            if (collidingEntities[COLLIDING_ABOVE] != null) {
                // Se está colidindo, ajusta a posição.
                pos.y = collidingEntities[COLLIDING_ABOVE].pos.y + collidingEntities[COLLIDING_ABOVE].pos.height;
                //Também zera a velocidade e a aceleração, para interromper o movimento.
                speed.y = 0;
                speed.x = 0;
            }
        } else if (speed.y > 0) {
            // Se a velocidade é maior do que zero, o objeto está caindo.
            // Testa a colisão ma direção abaixo.
            if (collidingEntities[COLLIDING_BELOW] != null) {
                // Se está colidindo, ajusta a posição.
                pos.y = collidingEntities[COLLIDING_BELOW].pos.y - pos.height + 1;
                // Também zera a velocidade e a aceleração,para interromper o movimento.
                speed.y = 0;
                accel.y = 0;
            }
        ...
        if (speed.x < 0) {
            // Se a velocidade é menor do que zero, o objeto está indo para a esquerda.
            // Testa colisão na direção esquerda.
            if (collidingEntities[COLLIDING_LEFT] != null) {
                // Se está colidindo, ajusta a posição.
                pos.x = collidingEntities[COLLIDING_LEFT].pos.x + collidingEntities[COLLIDING_LEFT].pos.width - 1;
                // Também zera a velocidade e a aceleração,para interromper o movimento.
                speed.x = 0;
                accel.x = 0;
            } else {
                // Se não está colidindo,deixa o movimento avançar,mas diminui ele com base na fricção.
                speed.x += friction;
            ...
            // Se a velocidade é maior do que zero, o objeto está indo para a direita.
            // Testa colisão na direção direita.
            if (collidingEntities[COLLIDING_RIGHT] != null) {
                // Se está colidindo,ajusta a posição.
                pos.x = collidingEntities[COLLIDING_RIGHT].pos.x - pos.width + 1;
                // Também zera a velocidade e a aceleração, para interromper o movimento.
                speed.x = 0;
                accel.x = 0;
            } else {
        ...
        // Muda a posição de acordo com a velocidade (que pode ser zero dependendo dos casos acima).
        pos.x += speed.x;
        pos.y += speed.y;
